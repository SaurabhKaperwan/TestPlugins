...
object Settings {
    ...
    fun showSettingsDialog(context: Context, onSave: () -> Unit) {
        ...
        // ── Stremio Addons ──
        layout.addView(createStremioAddonsCard(context))
    ...
    private fun createStremioAddonsCard(context: Context): View {
        ...
        toolbar.addView(pillBtn(context, "💾 Save All", ADDON_ACCENT, ADDON_BG, ADDON_BORDER) {
            val invalid = addons.indexOfFirst { it.url.isBlank() }
            if (invalid != -1) {
                Toast.makeText(context,
                    "Addon ${invalid + 1} has an empty URL", Toast.LENGTH_SHORT).show()
            } else {
                saveStremioAddons(addons)
                Toast.makeText(context, "✓ ${addons.size} addon(s) saved", Toast.LENGTH_SHORT).show()
            }
        })
        ...
        rebuildRows()
        content.addView(toolbar); content.addView(sep); content.addView(addonRows)
