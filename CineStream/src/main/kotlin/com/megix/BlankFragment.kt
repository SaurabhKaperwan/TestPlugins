...
class BlankFragment(private val plugin: ExamplePlugin) : BottomSheetDialogFragment() {
    ...
    override fun onViewCreated(
        ...
        val torrentToggle: Switch? = view.findViewByName("torrentToggle")
        ...
        // Handle torrent toggle
        torrentToggle?.apply {
            // Load saved state from SharedPreferences
            val sharedPrefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
            isChecked = sharedPrefs.getBoolean(KEY_TORRENT_ENABLED, false)

            // Set listener for toggle changes
            setOnCheckedChangeListener { _, isChecked ->
                // Save the new state
                sharedPrefs.edit().putBoolean(KEY_TORRENT_ENABLED, isChecked).apply()
                // Show feedback
                val message = if (isChecked) "Torrent Enabled" else "Torrent Disabled"
